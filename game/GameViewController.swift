...
//  betlead
//
//  Created by vanness wu on 2019/7/4.
...
class GameViewController : GameBaseViewController {
    ...
    enum GameType:Int,CaseIterable {
        ...
        var title:String {
            switch self {
            case .newest:
                return "最新".LocalizedString
            case .hot:
                return "熱門".LocalizedString
            case .played:
                return "曾经".LocalizedString
            case .favorite:
                return "最爱".LocalizedString
            case .filter:
                return "进阶".LocalizedString
            }
        }
    ...
    private let gameBgView: UIImageView = {
        let imv = UIImageView()
        imv.image = UIImage(named: "icon-monkey-light")
        imv.contentMode = .scaleAspectFill
        return imv
    }()
    private let menuView = UIView(color: .white)
    private let coverView = UIView(color: .white)
    private let topBgImv = UIImageView()
...
//    }
    ...
    private func bindStyle() {
        Themes.dreamWorkWhiteAndDarkBase.bind(to: menuView.rx.backgroundColor).disposed(by: disposeBag)
        Themes.dreamWorkWhiteAndDarkBase.bind(to: coverView.rx.backgroundColor).disposed(by: disposeBag)
        Themes.dreamWorkNavigationBg.bind(to: topBgImv.rx.image).disposed(by: disposeBag)
        Themes.dreamWorkMonkeyImage.bind(to: gameBgView.rx.image).disposed(by: disposeBag)
    }
    ...
    private func setupViews(){
        ...
        coverView.snp.makeConstraints { (maker) in
            ...
            coverTopConstraint = maker.top.equalTo(gameBgViewHeight).constraint
            maker.height.equalTo(Views.screenHeight)
        }
        
        topBgImv.snp.makeConstraints { (maker) in
            maker.top.leading.trailing.equalToSuperview()
    ...
    private func setupMenuView(){
        ...
        for gameType in GameType.allCases {
            ...
            btn.rx.tap.subscribeSuccess { (_) in
                Beans.matomoServer.track(eventWithCategory: MCategory.GameProperty.rawValue, action: MAction.Click.rawValue , name:gameType.title)
    ...
    private func selectedMenu(old:GameType, new:GameType) {
        ...
        let unselectColor = DreamWorkStyle.share.isLightMode() ? Themes.grayBase : .white
        let selectColor = DreamWorkStyle.share.isLightMode() ? Themes.primaryBase : Themes.secondaryYellow
        for btn in btns {
            btn.setTitleColor(unselectColor, for: .normal)
            btn.tintColor = unselectColor
        }
        btns[new.rawValue].setTitleColor(selectColor, for: .normal)
        btns[new.rawValue].tintColor = selectColor
    ...
    private func bindCollectionView(){
        Observable.merge(collectionViews.map { (view) -> Observable<CGFloat> in
            ...
            weakSelf.coverTopConstraint?.update(offset: weakSelf.gameBgViewHeight - offsetY)
            let ratio = abs(offsetY/weakSelf.maxOffsetY)

            weakSelf.topBgImv.alpha = ratio
        }.disposed(by: disposeBag)
